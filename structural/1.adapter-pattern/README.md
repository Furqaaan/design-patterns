...
Stripe SDK has pay($amount, $currency).
PayPal SDK has payment($amount, $currency).
Razorpay SDK might have charge($amount).
...
Defining a common interface: PaymentGateway.
Writing an Adapter for each provider that "translates" its unique methods into a unified method (processPayment()).
Controller now calls one method, no matter what the vendor SDK
